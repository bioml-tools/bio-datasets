...
from bio_datasets.structure.protein.utils import (
    compute_aligned_rmsd,
    load_backbone_coord_array,
)


def foldcomp_examples_generator(db_file, max_examples: Optional[int] = None):
...
def main(args):
    if args.dataset_type == "foldcomp":
        examples_generator = foldcomp_examples_generator(
    ...
    for atoms in examples_generator:
        ...
        compressor = load_bb_histogram_compressor(
            args.histogram_lib, sparse_bond_lengths=args.sparse
        )
        t0 = time.time()
        ...
        print(
            f"Bytes per residue: {len(compressed)/backbone_coords.shape[0]:.2f}, rmsd: {rmsd:.2f}"
        )
    ...
    print(
        f"Compression time: {np.mean(compression_times):.2f}, decompression time: {np.mean(decompression_times):.2f}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "histogram_lib",
        type=str,
        help="Path to saved histogram library to use for compression.",
    )
    parser.add_argument(
